 ...
 *
 *	 $Id: admin_ug_auth.php,v 9.16 2007/05/15 09:21:53 phoenix Exp $
 *
 *
 ***************************************************************************/
/***************************************************************************
...
if (isset($_POST['submit']) && (($mode == 'user' && $user_id) || ($mode == 'group' && $group_id))) {
	...
	if ($mode == 'user') {
		...
		//
		$result = $db->sql_query("SELECT g.group_id, u.user_level
		...
		if ($group_id < 1) {
			...
			$group_id = $db->sql_nextid('group_id');
			$db->sql_query("INSERT INTO ".USER_GROUP_TABLE." (user_id, group_id, user_pending) VALUES ($user_id, $group_id, 0)");
			$result = $db->sql_query("SELECT user_level FROM ".USERS_TABLE." WHERE user_id = $user_id");
			list($user_level) = $db->sql_fetchrow($result);
			$db->sql_freeresult($result);
		}
	}
	//
	...
	if ($mode == 'user' && $_POST['userlevel'] == 'admin' && $user_level != ADMIN) {
		...
		if ($userdata['user_id'] != $user_id) {
			$db->sql_query("UPDATE ".USERS_TABLE." SET user_level = ".ADMIN." WHERE user_id = $user_id");
			$db->sql_query("DELETE FROM ".AUTH_ACCESS_TABLE." WHERE group_id = '$group_id' AND auth_mod = 0");
			//
			// Delete any entries in auth_access, they are not required if user is becoming an admin
			//
			$db->sql_query("UPDATE " . AUTH_ACCESS_TABLE . "
		...
		if ($mode == 'user' && $_POST['userlevel'] == 'user' && $user_level == ADMIN) {
			...
			if ($userdata['user_id'] != $user_id) {
				...
				//
				$db->sql_query("UPDATE ".USERS_TABLE." SET user_level = ".USER." WHERE user_id = $user_id");
			}
			$message = $lang['Auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_userauth'], '<a href="'.adminlink("&amp;do=ug_auth&amp;mode=$mode").'">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="'.adminlink($op).'">', '</a>');
		} else {
			...
			$forum_access = array();
			while ($row = $db->sql_fetchrow($result)) { $forum_access[] = $row; }
			$db->sql_freeresult($result);

			if ($mode == 'user') {
				$result = $db->sql_query("SELECT aa.*
				...
				$result = $db->sql_query("SELECT * FROM ".AUTH_ACCESS_TABLE." WHERE group_id = '$group_id'");
			}
			$auth_access = array();
			while ($row = $db->sql_fetchrow($result)) { $auth_access[$row['forum_id']] = $row; }
			$db->sql_freeresult($result);
...
//						while (list($auth_type, $value) = each($update_acl_status[$forum_id])) {
					...
					$db->sql_query("INSERT INTO " . AUTH_ACCESS_TABLE . " (forum_id, group_id, $sql_field)
...
//					while ( list($auth_type, $value) = each($update_acl_status[$forum_id]) ) {
					...
					$db->sql_query("UPDATE " . AUTH_ACCESS_TABLE . " SET $sql_values
							WHERE group_id = '$group_id' AND forum_id = $forum_id");
				}
			}
			if ($delete_sql != '') {
				$db->sql_query("DELETE FROM " . AUTH_ACCESS_TABLE . " WHERE group_id = '$group_id' AND forum_id IN ($delete_sql)");
			}
			$l_auth_return = ( $mode == 'user' ) ? $lang['Click_return_userauth'] : $lang['Click_return_groupauth'];
			$message = $lang['Auth_updated'] . '<br /><br />' . sprintf($l_auth_return, '<a href="'.adminlink('&amp;do=ug_auth&amp;mode='.$mode).'">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="'.adminlink($op).'">', '</a>');
		}
		...
		//
		$result = $db->sql_query("SELECT u.user_id
		...
		while ($row = $db->sql_fetchrow($result)) { $set_mod[] = $row['user_id']; }
		$db->sql_freeresult($result);
		...
		while ($row = $db->sql_fetchrow($result)) { $unset_mod[] = $row['user_id']; }
		$db->sql_freeresult($result);
...
else if (($mode == 'user' && (isset($_POST['username']) || $user_id ) ) || ($mode == 'group' && $group_id))
{
	...
	//
	if ($mode == 'user') {
		$row = $db->sql_ufetchrow("SELECT user_id FROM " . USER_GROUP_TABLE . " WHERE user_id = $user_id LIMIT 0,1");
		if (!$row || $row['user_id'] != $user_id) {
			if (!($row = $db->sql_ufetchrow("SELECT MAX(group_id) AS total FROM ".GROUPS_TABLE))) {
				message_die(GENERAL_ERROR, 'Could not obtain next group_id information', '', __LINE__, __FILE__, $sql);
				return;
			}
			$group_id = $row['total'] + 1;
			$db->sql_query("INSERT INTO ".GROUPS_TABLE." (group_id, group_name, group_description, group_single_user, group_moderator)
				VALUES ($group_id, '', 'Personal User', 1, 0)");
			$db->sql_query("INSERT INTO " . USER_GROUP_TABLE . " (group_id, user_id, user_pending) VALUES ($group_id, $user_id, 0)");
		}
	}
	//
	...
	if ($mode == 'user') {
		$result = $db->sql_query("SELECT aa.*, g.group_single_user
			FROM ".AUTH_ACCESS_TABLE." aa, ".USER_GROUP_TABLE." ug, ".GROUPS_TABLE." g
			WHERE ug.user_id = $user_id
			  AND g.group_id = ug.group_id
			  AND aa.group_id = ug.group_id
	...
	foreach ($auth_ug as $forum_id => $user_ary) {
		...
		$template->assign_block_vars('forums', array(
			...
			'U_FORUM_AUTH' => adminlink("&amp;do=forumauth&amp;f=" . $forum_access[$i]['forum_id']),
	...
	if (count($name)) {
		...
		for ($i = 0; $i < count($ug_info); $i++) {
			...
			$t_usergroup_list .= (($t_usergroup_list != '') ? ', ' : '').'<a href="'.adminlink('&amp;do=ug_auth&amp;mode='.$ug.'='.$id[$i]).'">' . $name[$i] . '</a>';
		}
	} else {
	...
	$switch_mode = adminlink("&amp;do=ug_auth&amp;mode=$mode&amp;" . $u_ug_switch . "&amp;adv=$adv_switch");
	...
	$template->assign_vars(array(
		...
		'U_USER_OR_GROUP' => adminlink("&amp;do=ug_auth"),
		'U_SWITCH_MODE' => $u_switch_mode,

		'S_COLUMN_SPAN' => $s_column_span,
		'S_AUTH_ACTION' => adminlink("&amp;do=ug_auth"),
	...
	// Select a user/group
	//
	$tpl = ( $mode == 'user' ) ? 'forums/admin/user_select_body.html' : 'forums/admin/auth_select_body.html';
	...
	if ( $mode == 'user' ) {
		...
		$result = $db->sql_query("SELECT group_id, group_name FROM " . GROUPS_TABLE . " WHERE group_single_user <> " . TRUE);
		$select_list = '';
	...
	$template->assign_vars(array(
		...
		'S_HIDDEN_FIELDS' => $s_hidden_fields,
		'S_' . $l_type . '_ACTION' => adminlink("&amp;do=ug_auth")
		)
	);
}
