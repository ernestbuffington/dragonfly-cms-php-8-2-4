 ...
 *
 *	  $Id: admin_attach_cp.php,v 9.10 2007/05/15 09:21:53 phoenix Exp $
 *
 ***************************************************************************/
...
$view = isset($_POST['search']) ? 'attachments' : $view;
...
if ($confirm && (is_countable($delete_id_list) ? count($delete_id_list) : 0) > 0) {
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => adminlink('&amp;do=attach_cp'),
...
$template->assign_vars(array(
	...
	'S_MODE_ACTION' => adminlink('&amp;do=attach_cp'))
);
...
if ($submit_change && $view == 'attachments') {
	...
	//
	$result = $db->sql_query("SELECT c.cat_title, c.cat_id, f.forum_name, f.forum_id
		FROM " . CATEGORIES_TABLE . " c, " . FORUMS_TABLE . " f
		WHERE f.cat_id = c.cat_id ORDER BY c.cat_id, f.forum_order");
	$s_forums = '';
	...
	$template->set_filenames(array('body' => 'forums/admin/attach_cp_search.html'));
	...
	$template->set_filenames(array('body' => 'forums/admin/attach_cp_user.html'));
	$template->assign_vars(array(
	...
	if ( $num_members > 0 ) {
		for ($i = 0; $i < $num_members; $i++) {
			...
			//
			$result = $db->sql_query("SELECT attach_id FROM " . ATTACHMENTS_TABLE . " WHERE (user_id_1 = " . $members[$i]['user_id'] . ") GROUP BY attach_id");
		...
		for ($i = 0; $i < (is_countable($members) ? count($members) : 0); $i++) {
			...
			$template->assign_block_vars('memberrow', array(
				...
				'U_VIEW_MEMBER' => adminlink('&amp;do=attach_cp&amp;view=attachments&amp;uid=' . $members[$i]['user_id'])
				)
			);
		}
	}
	...
	//
	if ($user_based) {
		$result = $db->sql_query("SELECT username FROM " . USERS_TABLE . " WHERE user_id = " . $uid);
		$row = $db->sql_fetchrow($result);
		...
		$result = $db->sql_query("SELECT attach_id FROM " . ATTACHMENTS_TABLE . " WHERE user_id_1 = " . $uid . " GROUP BY attach_id");
		$attach_ids = $db->sql_fetchrowset($result);
		$num_attach_ids = $db->sql_numrows($result);
		...
		$sql = "SELECT a.* FROM " . ATTACHMENTS_DESC_TABLE . " a WHERE a.attach_id IN (" . implode(', ', $attach_id) . ") " . $order_by;
	}
		...
		$sql = "SELECT a.* FROM " . ATTACHMENTS_DESC_TABLE . " a " . $order_by;
	}
	...
	if ((is_countable($attachments) ? count($attachments) : 0) > 0)
	{
		for ($i = 0; $i < (is_countable($attachments) ? count($attachments) : 0); $i++)
		{
			...
			$template->assign_block_vars('attachrow', array(
				...
				'U_VIEW_ATTACHMENT' => getlink('&amp;file=download&amp;id=' . $attachments[$i]['attach_id'])
