...
class sql_db extends sql_parent
{
							...
							mysql_query('REPAIR TABLE '.$match[2], $this->connect_id)) 
		{
			...
			if ($this->query_result) {
				...
				return $this->query_result;
					...
					mysql_query("DELETE FROM $table[1] WHERE $table[2] LIKE '$entry[1]%' LIMIT ".(mysql_num_rows(mysql_query("SELECT $table[2] FROM $table[1] WHERE $table[2] LIKE '$entry[1]%'", $this->connect_id))-1), $this->connect_id)) ||
				(mysql_errno() == 1062 &&
					preg_match('#CREATE\s+UNIQUE\s+INDEX\s+([a-z_]+)\s+ON\s+([a-z_]+)\s+\(([a-z_]+)\)#i',$query,$table) &&
					preg_match("#Duplicate\s+entry\s+'(.*)'\s+for\s+key#i",mysql_error(),$entry) &&
					mysql_query("DELETE FROM $table[2] WHERE $table[3] LIKE '$entry[1]%' LIMIT ".(mysql_num_rows(mysql_query("SELECT $table[3] FROM $table[2] WHERE $table[3] LIKE '$entry[1]%'", $this->connect_id))-1), $this->connect_id)))
		{
	...
	function error()                { return array('message' => mysql_error($this->connect_id), 'code' => mysql_errno($this->connect_id)); }
