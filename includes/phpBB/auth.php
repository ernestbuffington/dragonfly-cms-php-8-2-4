 ...
 *
 *	 $Id: auth.php,v 9.4 2006/01/06 13:28:44 djmaze Exp $
 *
 *
 ***************************************************************************/
...
function session_pagestart()
{
	...
	if (can_admin($module_name)) { $userinfo['user_level'] = ADMIN; }
	$userdata =& $userinfo;
	return $userdata;
}
...
function auth($type, $forum_id, $userdata, $f_access = '')
{
 ...
 global $db, $lang;
	...
	{
		case AUTH_ALL:
			$a_sql = 'a.auth_view, a.auth_read, a.auth_post, a.auth_reply, a.auth_edit, a.auth_delete, a.auth_sticky, a.auth_announce, a.auth_vote, a.auth_pollcreate, a.auth_attachments, a.auth_download';
			$auth_fields = array('auth_view', 'auth_read', 'auth_post', 'auth_reply', 'auth_edit', 'auth_delete', 'auth_sticky', 'auth_announce', 'auth_vote', 'auth_pollcreate', 'auth_attachments', 'auth_download');
			break;
	...
	if (empty($f_access)) {
		...
		if ($forum_id != AUTH_LIST_ALL) {
			if (!($f_access = $db->sql_ufetchrow($sql))) {
				return array();
			}
		} else if (!($f_access = $db->sql_ufetchrowset($sql, SQL_BOTH)) ) {
			return array();
		}
	}
		...
		if ($forum_id != AUTH_LIST_ALL) {
			...
			for ($k = 0; $k < (is_countable($f_access) ? count($f_access) : 0); $k++) {
				...
				switch ($value) {
					...
					case AUTH_REG:
						$auth_user[$f_forum_id][$key] = is_user();
