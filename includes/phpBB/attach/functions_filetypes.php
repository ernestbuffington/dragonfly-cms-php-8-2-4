 ...
 *
 *	 $Id: functions_filetypes.php,v 9.1 2005/02/22 05:08:24 trevor Exp $
 *
 *
 ***************************************************************************/
...
function image_getdimension($file)
{
 ...
 $size = getimagesize($file);
	...
	if ($tmp_str == 'BM') {
		...
		if (!$error) {
			$i = read_longint($fp);
			if ( ($i != 0x3E) && ($i != 0x76) && ($i != 0x436) && ($i != 0x36) ) { $error = TRUE; }
		}
		if (!$error) {
			$tmp_str = fread($fp, 4); 
			$width = read_longint($fp); 
			$height = read_longint($fp);
			if ( ($width > 3000) || ($height > 3000) ) { $error = TRUE; }
		}
	} else {
		$error = TRUE;
	}
	...
	if ($tmp_str == 'GIF') {
		...
		if ( (($info_byte & 0x80) != 0x80) && (($info_byte & 0x80) != 0) ) { $error = TRUE; }
		if (!$error) {
			if (($info_byte & 8) != 0) { $error = TRUE; }
		}
	} else {
		$error = TRUE;
	}
	...
	$tmp_str = fread($fp, 4);
	$w1 = read_word($fp);
	if (intval($w1) < 16) { $error = TRUE; }
	if (!$error) {
		$tmp_str = fread($fp, 4);
		if ($tmp_str == 'JFIF') {
			$o_byte = fread($fp, 1);
			if (intval($o_byte) != 0) { $error = TRUE; }
			if (!$error) {
				$str = fread($fp, 2);
				$b = read_byte($fp);
				if ( ( $b != 0) && ($b != 1) && ($b != 2) ) { $error = TRUE; }
			}
			if (!$error) {
				$width = read_word($fp);
				$height = read_word($fp);
	...
	if ( ((ord($tmp_str[0]) == 10)) && ( (ord($tmp_str[1]) == 0) || (ord($tmp_str[1]) == 2) || (ord($tmp_str[1]) == 3) || (ord($tmp_str[1]) == 4) || (ord($tmp_str[1]) == 5) ) && (	   (ord($tmp_str[2]) == 1) ) ) {
		$b = fread($fp, 1);
		if ( (ord($b) != 1) && (ord($b) != 2) && (ord($b) != 4) && (ord($b) != 8) && (ord($b) != 24) ) { $error = TRUE; }
		if (!$error) {
			$xmin = read_word($fp);
			$ymin = read_word($fp);
			$xmax = read_word($fp);
