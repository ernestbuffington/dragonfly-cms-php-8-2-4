 ...
 *
 *	 $Id: functions_topics_list.php,v 9.5 2007/08/30 11:56:55 phoenix Exp $
 *
 *
 ***************************************************************************/
...
// various includes
include_once('includes/phpBB/functions_post.php');
include_once('includes/nbbcode.php');
//include_once('includes/functions_calendar.php');
//include_once('includes/functions_announces.php');
...
{
	global $db, $template, $board_config, $userdata, $lang, $images, $tree, $CPG_SESS, $module_name;
	static $box_id;
	...
	// choose template
	$template->set_filenames(array($tpl => 'forums/'.$tpl.'.html'));
	...
	for ($i=0; $i < (is_countable($topic_rowset) ? count($topic_rowset) : 0); $i++)
	{
		...
		{
			case POST_USERS_URL:
				$view_topic_url		= getlink("&amp;file=profile&amp;" . POST_USERS_URL . "=$topic_id");
				break;
			default:
				...
				$last_post_url		= '<a href="' . getlink("&amp;file=viewtopic&amp;"	. POST_POST_URL . '=' . $topic_rowset[$i]['topic_last_post_id']) . '#' . $topic_rowset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" /></a>';
		...
		if ( $display_nav_tree && !empty($topic_rowset[$i]['forum_id']) )
		{
			...
			else
			{
				...
				{
					$nav_tree = '<a href="' . getlink("&amp;file=viewforum&amp;f=" . $topic_rowset[$i]['forum_id']) . '" class="gensmall">' . $topic_rowset[$i]['forum_name'] . '</a>';
				}
			}
		}
		if (!empty($nav_tree))
