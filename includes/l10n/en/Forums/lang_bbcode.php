...
$faq[] = array("Linking to another site", "BBCode supports a number of ways of creating URIs, Uniform Resource Indicators better known as URLs.<ul><li>The first of these uses the <b>[url=][/url]</b> tag; whatever you type after the = sign will cause the contents of that tag to act as a URL. For example, to link to {$_SERVER['HTTP_HOST']} you could use:<br /><br /><b>[url=http://{$_SERVER['HTTP_HOST']}/]</b>Visit {$_SERVER['HTTP_HOST']}!<b>[/url]</b><br /><br />This would generate the following link, <a href=\"http://{$_SERVER['HTTP_HOST']}/\" target=\"_blank\">Visit {$_SERVER['HTTP_HOST']}!</a> You will notice the link opens in a new window so the user can continue browsing the forums if they wish.</li><li>If you want the URL itself displayed as the link you can do this by simply using:<br /><br /><b>[url]</b>http://{$_SERVER['HTTP_HOST']}/<b>[/url]</b><br /><br />This would generate the following link: <a href=\"http://{$_SERVER['HTTP_HOST']}/\" target=\"_blank\">http://{$_SERVER['HTTP_HOST']}/</a></li><li>Additionally {$_SERVER['HTTP_HOST']} features something called <i>Magic Links</i>which will turn any syntatically correct URL into a link without you needing to specify any tags or even the leading http://. For example typing {$_SERVER['HTTP_HOST']} into your message will automatically lead to <a href=\"http://{$_SERVER['HTTP_HOST']}/\" target=\"_blank\">{$_SERVER['HTTP_HOST']}</a> being output when you view the message.</li><li>The same thing applies equally to email addresses; you can either specify an address explicitly, like:<br /><br /><b>[email]</b>no.one@example.com<b>[/email]</b><br /><br />which will output <a href=\"emailto:no.one@example.com\">no.one@example.com</a> or you can just type no.one@example.com into your message and it will be automatically converted when you view.</li></ul>As with all the BBCode tags you can wrap URLs around any of the other tags such as <b>[img][/img]</b> (see next entry), <b>[b][/b]</b>, etc. As with the formatting tags it is up to you to ensure the correct open and close order is following. For example:<br /><br /><b>[url=http://{$_SERVER['HTTP_HOST']}/][img]</b>http://{$_SERVER['HTTP_HOST']}/images/phplogo.gif<b>[/url][/img]</b><br /><br />is <u>not</u> correct which may lead to your post being deleted so take care.");

$faq[] = array("--", "Showing images in posts");
$faq[] = array("Adding an image to a post", "BBCode incorporates a tag for including images in your posts. Two very important things to remember when using this tag are: many users do not appreciate lots of images being shown in posts and second, the image you display must already be available on the Internet (it cannot exist only on your computer, for example, unless you run a webserver!). There is currently no way of storing images locally with {$_SERVER['HTTP_HOST']} (all these issues are expected to be addressed in the next release of {$_SERVER['HTTP_HOST']}). To display an image, you must surround the URL pointing to the image with <b>[img][/img]</b> tags. For example:<br /><br /><b>[img]</b>http://{$_SERVER['HTTP_HOST']}/images/logo.png<b>[/img]</b><br /><br />As noted in the URL section above you can wrap an image in a <b>[url][/url]</b> tag if you wish, eg.<br /><br /><b>[url=http://{$_SERVER['HTTP_HOST']}/][img]</b>http://{$_SERVER['HTTP_HOST']}/images/logo.png<b>[/img][/url]</b><br /><br />would generate:<br /><br /><a href=\"http://{$_SERVER['HTTP_HOST']}/\"
 target=\"_blank\"><img src=\"images/logo.png\" alt=\"\" /></a><br />");

$faq[] = array("--", "Other matters");
$faq[] = array("Can I add my own tags?", "No, I am afraid not.");
