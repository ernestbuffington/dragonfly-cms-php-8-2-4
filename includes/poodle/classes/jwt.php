...
abstract class JWT
{
	 ...
	 */
	public static function decode($jwt, $key, $timestamp = null, $leeway = 0)
	{
		...
		try {
			$header = JSON::decode(Base64::urlDecode($jwt[0]), JSON_BIGINT_AS_STRING);
		} catch (\Exception $e) {
			throw new \UnexpectedValueException("Invalid header encoding ({$e->getMessage()})");
		}
		try {
			$payload = JSON::decode(Base64::urlDecode($jwt[1]), JSON_BIGINT_AS_STRING);
		} catch (\Exception $e) {
			throw new \UnexpectedValueException("Invalid claims encoding ({$e->getMessage()})");
		}
		$sig = Base64::urlDecode($jwt[2]);
	 ...
	 */
	public static function encode($payload, $key, $alg = 'HS256', $keyId = null, array $header = array())
	{
		...
		$segments = array(
			Base64::urlEncode(JSON::encode($header)),
			Base64::urlEncode(JSON::encode($payload))
		);
		$segments[] = Base64::urlEncode(static::sign(implode('.', $segments), $key, $alg));
	 ...
	 */
	public static function sign($msg, $key, $alg = 'HS256', $passphrase = '')
	{
		switch ($alg)
		{
			case 'RS256':
			...
			case 'HS512':
			case 'HS384':
				return Hash::hmac('SHA'.substr($alg,2), $msg, $key, true);
	...
	private static function verify($msg, $signature, $key, $alg)
	{
		switch ($alg)
		{
			...
			case 'HS512':
			case 'HS384':
				$hash = Hash::hmac('SHA'.substr($alg,2), $msg, $key, true);
				return Hash::equals($signature, $hash);
