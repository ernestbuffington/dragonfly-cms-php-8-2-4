...
while ($row = $result->fetch_assoc()) {
	...
	Coppermine::truncateString($thumb_title, 15);
	$views_title = \Dragonfly::getKernel()->L10N->plural($row['hits'],'%d views');
	$content .= '<a href="' . htmlspecialchars($CPG->getImageMetaUrl('topn', $pic)) . '"><img src="' . $CPG->getPicSrc($row, 'thumb') . '" alt="' . $thumb_title . '" title="' . $thumb_title . '" /><br />' . $views_title . '</a><br /><br />';
	++$pic;
}
