...
while ($row = $result->fetch_assoc()) {
	...
	$content .= '<a href="' .htmlspecialchars($CPG->getImageUrl($row)). '"><img src="' . $CPG->getPicSrc($row, 'thumb') . '" alt="' . $thumb_title . '" title="' . $thumb_title . '" /><br />'
		. Coppermine::truncateString($thumb_title,15) . '</a><br />';
}
$content .= '<br /><a href="' . URL::index("$cpg_dir") . '">'._coppermineLANG.'</a></p>';
