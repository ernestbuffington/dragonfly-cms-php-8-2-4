...
$result = $db->query("SELECT pid, filepath, filename, p.aid, p.title
	...
	WHERE approved=1 ORDER BY pid DESC LIMIT {$limit}");
$pic = 0;
...
while ($row = $result->fetch_assoc()) {
	...
	$content .= '<td align="center" valign="baseline"><a href="' . htmlspecialchars($CPG->getImageMetaUrl('lastup', $pic)) . '"><img src="' . $CPG->getPicSrc($row, 'thumb') . '" alt="' . $thumb_title . '" title="' . $thumb_title . '" /><br />' . Coppermine::truncateString($thumb_title, 20) . '</a>&nbsp;&nbsp;</td>';
	++$pic;
}
$content .= '</tr></table></marquee></p><p style="text-align:center;"><table><tr align="center"><td valign="baseline"><a href="' . URL::index($cpg_dir) . '">' . _coppermineLANG . '</a><br /></td></tr></table></p>';
