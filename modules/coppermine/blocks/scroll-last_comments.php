...
$result = $db->query("SELECT p.pid, p.title, filepath, filename, msg_author, msg_date, msg_body
...
while ($row = $result->fetch_assoc()) {
	...
	$content .= '<a href="'.htmlspecialchars($CPG->getImageMetaUrl('lastcom', $pic)).'"><img src="'.$CPG->getPicSrc($row, 'thumb').'" style="border:0;" alt="'.$thumb_title.'" title="'.$thumb_title.'" /><br />'.Coppermine::truncateString($row['msg_author'], 10).'</a> <br />'.Coppermine::truncateString($row['msg_body'], 50).'<br />('.$date.')<br /><br />';
	++$pic;
}
