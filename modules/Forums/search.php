 ...
 *
 *	 $Id: search.php,v 9.14 2007/12/12 12:54:24 nanocaiordo Exp $
 *
 *
 ***************************************************************************/
...
if ( !empty($_POST['search_time']) || !empty($_GET['search_time'])) {
	$search_time = gmtime() - ( ( ( !empty($_POST['search_time']) ) ? intval($_POST['search_time']) : intval($_GET['search_time']) ) * 86400 );
...
if ( $mode == 'searchuser' ) {
	...
	if ( isset($_POST['search_username']) ) {
		if (!preg_match('#\*#mi', $_POST['search_username'])) $_POST['search_username'] = "*".$_POST['search_username']."*";
	...
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' ) {
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' )  ) {
			...
			$split_search = ( !strstr($multibyte_charset, (string) $lang['ENCODING']) ) ?  split_words(clean_words('search', $search_keywords, $stopword_array, $synonym_array), 'search') : preg_split('# #m', $search_keywords);
				...
				switch ( $split_search[$i] )
				{
					...
					default:
						...
						if ( !strstr($multibyte_charset, (string) $lang['ENCODING']) ) {
							...
							$match_word = '%'.Fix_Quotes(str_replace('*', '', $split_search[$i])).'%';
           ...
           if (!isset($row[$post_id]) || !$row[$post_id]) { $result_list[$post_id] = 0; }
       }
						}
		...
		*/
		...
		for($i = 0; $i < count($store_vars); $i++) {
			$CPG_SESS[$module_name]['search'][$search_id][$store_vars[$i]] = ${$store_vars}[$i];
		}
	} else {
		$search_id = intval($search_id);
		if ( $search_id ) {
			if (!is_array($CPG_SESS[$module_name]['search'][$search_id])) {
				message_die(GENERAL_ERROR, 'Could not obtain search results', '', __LINE__, __FILE__, $sql);
			}
			for($i = 0; $i < count($store_vars); $i++) {
				${$store_vars}[$i] = $CPG_SESS[$module_name]['search'][$search_id][$store_vars[$i]];
			}
		}
	}
	...
	if ( $search_results != '' ) {
		...
		for($j = 0; $j < (is_countable($split_search) ? count($split_search) : 0); $j++ ) {
			...
			if ( $split_word != 'and' && $split_word != 'or' && $split_word != 'not' ) {
				...
				for ($k = 0; $k < (is_countable($synonym_array) ? count($synonym_array) : 0); $k++) {
					list($replace_synonym, $match_synonym) = preg_split('# #m', trim(strtolower($synonym_array[$k])));
		...
		for($i = 0; $i < count($searchset); $i++) {
			$forum_url = getlink("&amp;file=viewforum&amp;" . POST_FORUM_URL . '=' . $searchset[$i]['forum_id']);
			$topic_url = getlink("&amp;file=viewtopic&amp;" . POST_TOPIC_URL . '=' . $searchset[$i]['topic_id'] . (!empty($highlight_active) ? "&amp;highlight=$highlight_active" : ""));
			if (isset($searchset[$i]['post_id'])) {
				$post_url = getlink("&amp;file=viewtopic&amp;" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . (!empty($highlight_active) ? "&amp;highlight=$highlight_active" : "")) . '#' . $searchset[$i]['post_id'];
			} else {
				$post_url = '';
			}
			$post_date = create_date($board_config['default_dateformat'], $searchset[$i]['post_time']);
			...
			if ($show_results == 'posts') {
							...
							if ( preg_match('/<.*>/', $message) ) {
								...
								while( $start_html = strpos($message, '<', $start_html) )
								{
									...
									if ( $end_html = strpos($message, '>', $start_html) )
									{
										...
										{
											$hold_string = str_replace('<!-- #sh -->', '<span style="color:#FFA34F;"><b>', $hold_string);
										}
				...
				$poster = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . getlink("Your_Account&amp;profile=" . $searchset[$i]['user_id']) . '">' : '';
			...
			else
			{
						...
						if ($searchset[$i]['post_time'] > $userdata['user_lastvisit']) {
							if (!empty($tracking_topics) || !empty($tracking_forums) || isset($CPG_SESS[$module_name]['track_all'])) {
								...
								if (isset($CPG_SESS[$module_name]['track_all'])) {
									if ($CPG_SESS[$module_name]['track_all'] > $searchset[$i]['post_time']) {
										$unread_topics = false;
									}
								}
				...
				$topic_author = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . getlink('Your_Account&amp;profile=' . $searchset[$i]['user_id']) . '">' : '';
				...
				$first_post_time = create_date($board_config['default_dateformat'], $searchset[$i]['topic_time']);

				$last_post_time = create_date($board_config['default_dateformat'], $searchset[$i]['post_time']);

				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . getlink('Your_Account&amp;profile=' . $searchset[$i]['id2']) . '">' . $searchset[$i]['user2'] . '</a>';

				$last_post_url = '<a href="' . getlink("&amp;file=viewtopic&amp;"  . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" /></a>';
		...
		$template->assign_vars(array(
			'PAGINATION' => generate_pagination("&amp;file=search&amp;search_id=$search_id", $total_match_count, $per_page, $start),
		...
		if ( $show_results == 'posts' ) {
			$template->set_filenames(array('body' => 'forums/search_results_posts.html'));
		} else {
			$template->set_filenames(array('body' => 'forums/search_results_topics.html'));
		}
...
$template->assign_vars(array(
	...
	'S_SEARCH_ACTION' => getlink("&amp;file=search&amp;mode=results"),
